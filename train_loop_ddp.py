...
def train_loop(train_loader, test_loader, net, opt, lr_schd, output_dir, args, device, log, rank):
    train_epoch_losses=[]
    for epoch in range(args.start_epoch, args.max_epoch):
        ...
        log.flush()
        if rank==0:
            save_checkpoint(state={'net': net.module.state_dict(), 'opt': opt.state_dict(), 'epoch': epoch}, path=os.path.join(output_dir, 'epoch-{}-checkpoint.pth'.format(epoch)))
